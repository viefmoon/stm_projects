...
void wireWriteRegister (uint8_t reg, uint16_t value)
{
    ...
    i2c_temp[0] = value>>8;
    i2c_temp[1] = value;
    HAL_I2C_Mem_Write(&hi2c3, adrs_219<<1, (uint16_t)reg, 1, i2c_temp, 2, 0xffffffff);
    HAL_Delay(1);
}
...
{
    uint8_t i2c_temp[2];
    HAL_I2C_Mem_Read(&hi2c3, adrs_219<<1, (uint16_t)reg, 1,i2c_temp, 2, 0xffffffff);
    HAL_Delay(1);
...
int contMeasureInit(uint8_t reg) {
    ...
    status = HAL_I2C_Master_Transmit(&hi2c3, adrs_219 << 1, &reg, 1,
            0xffffffff);
    if (status != HAL_OK)
        while (1)
            ;
...
int contMeasureUpdate(void) {
    ...
    status = HAL_I2C_Master_Receive(&hi2c3, adrs_219 << 1,
